...
below are provided and configured.

> **This list now lives in the app.** Go to **Administration → Sale documents setup**
> (`/admin/documents`). It shows all twelve items with live status, and it is where you
> upload approved templates. Anything the app can detect for itself — is the registry
> loaded, is the dealer address filled in, how many templates are loaded — it works out
> rather than asking. This file is the reference; the app is the working copy.

**Loading approved templates is self-service.** Upload the counsel-approved copy of a
document and it stops generating a watermark — no code change, no deploy. Do them one at
a time as they come back. Only documents the dealership produces (category 1) and blank
government forms it prints (category 2) take an upload: a REG 51 is serialised and issued
by the DMV, and a smog certificate belongs to a specific car and is attached to that sale
on the deal's checklist.
